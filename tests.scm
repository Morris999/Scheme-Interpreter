...
`(define pi (+ 3 0.14))
;expect (define pi (+ 3 0.14))

(define a `(define pi ,(+ 3 0.14)))
;expect a

a
;expect (define pi 3.14)
...
(begin (define a 3) (define a 5) (+ a 2))
;expect 7

(begin (define a 999) (define a (begin (+ 9 9) (* 9 9))) (* a 1))
;expect 81
...
;PROBLEM 10 TESTS
(define (return-nine x) '(9))
;expect return-nine

(return-nine 3)
;expect (9)

(define (last-thing x) (begin (* x x) (+ x x)))
;expect last-thing
...
(define always-bye (lambda () (begin 'hi 'bye)))
;expect always-bye

always-bye
;expect (lambda () (begin (quote hi) (quote bye)))
...
(and (define a 3) 3 #f (define (a x) (+ x x)))
;expect #f
...
(let ((x 2)) (let ((y (+ x 1))) (define z (* y 2))))
;expect z
z
;expect Error
...
(define (g) (begin (define x 9) (+ x 3) (f)))
;expect g
...
;PROBLEM 20 TESTS
(define (db-n-times n x)
  (if (= n 0)
    x
    (* 2 (db-n-times (- n 1) x)))
)
(define (db-n-times-tail n x)
  (define (helper n res)
    (if (= n 0)
      res
      (db-n-times (- n 1) (* 2 res)))
  )
  (helper n x)
)
...
;PROBLEM 21 TESTS
(define-macro (db-if-true value expr)
  `(if ,value
      (* ,expr 2)
      ,expr
    )
