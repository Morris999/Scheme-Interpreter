...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals and not vals or not formals and vals:
            raise SchemeError
        return child_frame
        # END PROBLEM 11
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        macro_stuff = Pair(target.second, expressions.second)
        check_form(macro_stuff, 2)
        check_formals(macro_stuff.first)
        macro_func = MacroProcedure(macro_stuff.first, macro_stuff.second, env)
        env.define(target.first, macro_func)
        return target.first
    else:
        raise SchemeError('bad target')
    # END Problem 21
